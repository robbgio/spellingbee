...
function logKey(e) {
    ...
    if (isLetter(typedArray[0])) typedLetter = typedArray[0].toLowerCase();
...
function checkSpelling(typedLetter1) {
    currentTypedWord.push(typedLetter1);
    ...
    if (currentTypedWord[currentTypedIndex] === currentWordArray[currentTypedIndex].toLowerCase()) {
        ...
        if (currentWordArray.length === currentTypedIndex + 1) {
            ...
            if (numberOfTries === 0) {
                ...
                currentStreak++;
                newBalloon();
                currentStreak > highStreak ? highStreak = currentStreak : highStreak;
        ...
        setTimeout(function() {
            document.getElementById("typed-word-box").style.background = "none";
        }, 2000);
        //first fail
        if (numberOfTries < 2) {
            currentTypedIndex = 0;
...
function flashWord(currentWord) {
    setTimeout(function() {
        setSayWordButtonText(currentWord);
    }, 1500);
    setTimeout(function() {
        setSayWordButtonText("Say Word");
    }, 3500);
    setTimeout(function() {
        sayWord(currentWord);
    }, 1000);
...
function playSound() {
    var audio = new Audio("/sounds/flexatone-cartoon-wobble-ni-sound-1-00-02.mp3");
    audio.play();
}
...
function getWordsArray() {
    if (localStorage.hasOwnProperty("wordsArray")) return JSON.parse(localStorage.getItem("wordsArray"));
    else return null;
}
...
function getHighStreak() {
    if (localStorage.hasOwnProperty("highStreak")) return JSON.parse(localStorage.getItem("highStreak"));
    else return null;
}
function newBalloon() {
...
function getRandomCoordinates() {
    return [
        Math.floor(Math.random() * (canvasWidth - balloonRadius * 2) + balloonRadius),
